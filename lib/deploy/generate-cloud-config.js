...
module.exports = async function ({deploymentDefinition, decisions, credentials}) {
	const cloudConfig = new CloudConfig(path.resolve(TEMPLATES_FOLDER, 'cloud-config.yml'));
	...
	let mountPath;
	const formatAndMountVolumeScript = fs.readFileSync(path.resolve(TEMPLATES_FOLDER, 'format-and-mount-volume.sh'), 'utf-8');
	...
	const nginxSiteTemplate = deploymentDefinition.secure ?
		fs.readFileSync(path.resolve(TEMPLATES_FOLDER, 'nginx-site-secure.mst'), 'utf-8') :
		fs.readFileSync(path.resolve(TEMPLATES_FOLDER, 'nginx-site-insecure.mst'), 'utf-8');
	...
	if (deploymentDefinition.secure) {
		...
		cloudConfig.addWriteFile({
			...
			content: fs.readFileSync(path.resolve(TEMPLATES_FOLDER, 'letsencrypt-cronjob'), 'utf-8')
		});
	...
	const installNodeCGDepdenciesScript = fs.readFileSync(path.resolve(TEMPLATES_FOLDER, 'install-nodecg-dependencies.sh'), 'utf-8');
	cloudConfig.addWriteFile({
		owner: 'root:root',
