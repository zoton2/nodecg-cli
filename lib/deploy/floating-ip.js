...
async function selectAction(message, digitalOceanApi, decisions) {
	...
	if (floatingIpAction === 'create') {
		const response = await digitalOceanApi.createFloatingIP({region: decisions.chosenRegion});
		return response.ip;
	}

	if (floatingIpAction === 'choose_existing') {
		return chooseExistingFloatingIP(digitalOceanApi, decisions);
	}
